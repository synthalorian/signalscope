...
pub fn capture_samples(config: &CaptureConfig, count: usize) -> Result<Vec<IQSample>> {
    ...
    device.set_center_freq(config.center_freq as u32)
    ...
    if config.gain > 0 {
        ...
        device.set_tuner_gain(config.gain)
...
pub fn capture_samples(_config: &CaptureConfig, _count: usize) -> Result<Vec<IQSample>> {
    Err(anyhow!("RTL-SDR support not compiled in. Rebuild with --features rtlsdr"))
}
