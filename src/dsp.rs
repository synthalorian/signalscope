...
pub fn detect_peaks(result: &FFTResult, sample_rate: f64, center_freq: f64, config: &PeakDetectConfig) -> Vec<Peak> {
    ...
    for i in config.noise_window_bins..(n - config.noise_window_bins) {
        ...
        let is_local_max = (i.saturating_sub(config.min_distance_bins)..i)
            ...
            && ((i + 1)..=(i + config.min_distance_bins).min(n - 1))
            .all(|j| bins[j] <= current);
        ...
        let noise_floor = if noise_samples.len() % 2 == 0 {
    ...
    peaks.sort_by(|a, b| b.amplitude_db.partial_cmp(&a.amplitude_db).unwrap_or(std::cmp::Ordering::Equal));
    peaks.truncate(config.max_peaks);
    peaks.sort_by(|a, b| a.frequency_hz.partial_cmp(&b.frequency_hz).unwrap_or(std::cmp::Ordering::Equal));
...
pub fn auto_threshold(result: &FFTResult) -> f32 {
    ...
    for t in 1..HIST_BINS {
        ...
        let mu0: f64 = histogram[..t].iter().enumerate()
            .map(|(i, &c)| i as f64 * c as f64)
            .sum::<f64>() / (w0 * total);
        let mu1: f64 = histogram[t..].iter().enumerate()
